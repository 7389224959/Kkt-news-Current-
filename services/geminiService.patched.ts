...
export const generateFullReelScript = async (
  ...
  const prompt = `# ROLE
You are the senior short-form news editor and Hindi digital anchor for KKT News.
Convert the supplied article into a factual, natural, high-retention Facebook and Instagram Reel script for a Chhattisgarh/local-news audience.

# CORE STANDARD
Accuracy always comes before virality. Use only facts present in the article. Never invent names, motives, causes, quotes, numbers, evidence, police action, or outcomes.

# STORY-TYPE SELECTION
First classify the story as exactly one of:
- crime
- accident
- public_utility
- politics
...
Use the matching progression:
- crime: strongest development -> essential evidence/context -> police/current status
- accident: consequence -> location/cause if verified -> rescue/current condition
- public_utility: exact change -> who is affected -> date/action citizens should take
- politics: decision/statement -> public impact -> verified response
- explainer: direct problem -> two or three essential facts -> useful conclusion
- human_interest: surprising outcome -> action/struggle -> payoff
- sports: result -> decisive moment -> next consequence
- entertainment: verified development -> why it matters -> confirmed next step

# RETENTION RULES
1. Default to 45-65 spoken words, designed for roughly 18-28 seconds. Use up to 75 words only when essential verified facts genuinely require it.
2. The first 8-12 words must reveal the strongest verified consequence, conflict, decision, result, or unresolved development.
3. Do not open with filler or stale TV phrases such as "badi khabar", "breaking news", "sansanikhej mamla", "hadkamp mach gaya", "mamla saamne aaya", "dosto", or "agar aap".
4. Add a genuinely new fact or development in every narration beat. Never repeat the same fact in different words.
5. Use short, conversational Hindi/Hinglish sentences. Prefer active voice and natural spoken phrasing.
6. Mention only essential names in narration. Put secondary names, designations, dates, and details in on-screen text.
7. Never use bracketed delivery tags or stage directions.
8. Never ask viewers to decide guilt, punishment, politics, or morality.
9. Do not manufacture suspense. A curiosity gap is allowed only when the answer is delivered later in the same reel.
10. Avoid exaggerated adjectives unless directly supported by verified facts.
11. A spoken CTA is optional. Use it only for an ongoing/developing story, and keep it to 8 words or fewer. Otherwise end on the strongest status or consequence.
12. The completed voiceover must contain 3 or 4 logical narration beats.

# HOOK SELECTION
Create two factual candidate hooks internally:
- Hook A: consequence/result-led
- Hook B: conflict/unresolved-status-led
Select the stronger hook using specificity, local relevance, clarity, and curiosity. Return both candidates for testing, but voiceoverScript must begin with selectedHook exactly.

# VISUAL BEATS
Return exactly 3 or 4 visual beats aligned to the narration. Each beat must:
- contain the exact consecutive spoken words covered by that beat;
- state what should be visible, not abstract keywords;
- prefer verified real footage/photos, identifiable locations, officials, documents, maps, or relevant licensed B-roll;
- avoid generic crime tape, random police, unrelated crowds, logos, illustrations, and repeated images.
...
{
  "storyType": "crime | accident | public_utility | politics | explainer | human_interest | sports | entertainment",
  "hookCandidates": {
    "consequenceHook": "8-12 spoken words",
    "conflictHook": "8-12 spoken words"
  },
  "selectedHook": "the selected hook exactly as used at the beginning of voiceoverScript",
  ${hasHeadline ? '"headline": "specific on-screen hook, maximum 8 words",' : ""}
  "voiceoverScript": "45-65 spoken words by default; maximum 75 only when essential",
  ${hasSubtitles ? '"subtitleChunks": ["3-5 exact consecutive words from the voiceover"],' : ""}
  ${hasTicker ? '"ticker": "specific verified update, maximum 50 characters",' : ""}
  "visualBeats": [
    {
      "beat": 1,
      "narration": "exact consecutive words from voiceoverScript",
      "purpose": "hook | reveal | development | status",
      "visualQuery": "concrete visual that should appear",
      "onScreenText": "maximum 6 words",
      "preferredSource": "article_media | original_kkt | official | licensed_stock | map_or_document"
    }
  ],
  "reelType": "Breaking News | Explainer | Useful Update | Human Interest",
  "stylePreset": "breaking_news | explainer | useful_update | human_interest",
  "facebookCaption": "Natural factual caption of 150-300 characters with 1-3 relevant hashtags",
  "onScreenHookText": "maximum 6 words",
  "strategicCtaQuestion": "optional non-question CTA of 8 words or fewer, or empty string"
}

# FINAL CHECK BEFORE OUTPUT
- Verify selectedHook is the exact beginning of voiceoverScript.
- Verify the script contains 45-65 words unless essential facts require up to 75.
- Verify visualBeats contain the entire voiceover in order without rewriting or omissions.
- Remove filler, repetition, stage directions, moral questions, and unsupported claims.
- Return JSON only.`;
  ...
  try {
    ...
    if (result.voiceoverScript) {
      result.voiceoverScript = cleanVoiceoverScript(result.voiceoverScript, false)
        .replace(/^(🚨\\s*)?(बड़ी खबर|ब्रेकिंग न्यूज़|सनसनीखेज मामला|हड़कंप मच गया|मामला सामने आया|badi khabar|breaking news|sansanikhej mamla|hadkamp mach gaya|mamla saamne aaya)\\s*[:–—-]?\\s*/i, "")
        .trim();

      const wordCount = result.voiceoverScript.split(/\\s+/).filter(Boolean).length;
      result.durationTargetSeconds = wordCount <= 55 ? 22 : wordCount <= 65 ? 26 : 30;

      if (!result.selectedHook || !result.voiceoverScript.startsWith(result.selectedHook)) {
        const firstSentence = result.voiceoverScript.split(/(?<=[.!?।])\\s+/)[0]?.trim();
        result.selectedHook = firstSentence || result.voiceoverScript.split(/\\s+/).slice(0, 12).join(" ");
      }
    }

    // Keep the old field for UI compatibility, but never allow a judgement question.
    if (!result.strategicCtaQuestion || /saza|phaansi|guilt|aaropi.*kadi|\\?$/i.test(result.strategicCtaQuestion.trim())) {
      result.strategicCtaQuestion = "";
    }

    if (!Array.isArray(result.visualBeats)) result.visualBeats = [];
...
export const generateReelAudio = async (
  ...
  const ttsPrompt = `Read the following Hindi/Hinglish news script like a credible Indian digital journalist speaking naturally to one viewer.
Use clear articulation, controlled energy, and conversational authority. Start immediately with the first fact—do not add an introduction.
Do not sound like a shouting TV debate anchor. Match emotion to the story: restrained for tragedy and crime, practical for public updates, warm for human-interest stories, and energetic only for sports or entertainment.
Use natural pauses at punctuation, emphasize names, locations, numbers, and the newest development, and keep a steady mobile-friendly pace of roughly 165-180 words per minute.
Do not read emojis, hashtags, stage directions, or quotation marks aloud.
