...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ignoreWords = ['subscribe', 'login', 'advertisement', 'video'];

    // Check if it's markdown (Jina AI might return markdown despite headers)
    if (!html.trim().startsWith('<') && html.includes('](')) {
      ...
      while ((match = regex.exec(html)) !== null) {
        ...
        if (ignoreWords.some(word => lowerTitle.includes(word))) continue;
      ...
      return res.status(200).json({ links: results });
    }
    ...
    $("a").each((i, el) => {
      ...
      // Ignore links containing specific words
      if (ignoreWords.some(word => lowerHref.includes(word) || lowerTitle.includes(word))) {
        return;
      }
    ...
    res.status(500).json({ error: error.message || 'Failed to extract links' });
  }
}
