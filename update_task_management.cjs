...
const regex = /function TaskManagement\(\{ workerId, workerTasks, allTasks, onJoinTask, onUpdateTaskStatus \}: \{ workerId\?: string, workerTasks: any\[\], allTasks: any\[\], onJoinTask: \(id: string\) => void, onUpdateTaskStatus: \(id: string, status: string\) => void \}\) \{([\s\S]*?)(?=export const WorkerDashboard)/;

const newFunc = `function TaskManagement({ workerId, workerTasks, allTasks, onJoinTask, onUpdateTaskStatus }: { workerId?: string, workerTasks: any[], allTasks: any[], onJoinTask: (id: string) => void, onUpdateTaskStatus: (id: string, status: string) => void }) {
            ...
            <div className="p-6 flex flex-col h-full">
              ...
              <div className="grid grid-cols-2 gap-2 mt-auto">
                ...
                {taskFilter === 'Available' ? (
                  <button onClick={() => { onJoinTask(task.id); setActiveTask({...task, status: 'Pending'}); }} className="w-full py-2 text-sm font-medium rounded-lg transition-colors bg-blue-600 text-white hover:bg-blue-700">
                    Join Task
                  </button>
                ) : (
                  <button 
